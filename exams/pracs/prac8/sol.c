...
void drawAllPaths(int src, int *st, int ncities) {
    ...
    for (i = 0; i < ncities; i++) {
        c = 0;
        currVertex = i;
        // loop backwards into path
        while (st[currVertex] != -1 && currVertex != src && c < ncities) {
            // fill in path
            path[c++] = currVertex; 
            // go to previous
            currVertex = st[currVertex];
        }

        // no path found...
        if (st[currVertex] == -1 || c >= ncities) {
            printf("no path found\n");    
        } else {
            path[c] = src;
            for (currVertex = c; currVertex >= 0; currVertex--) {
                printf("[%d]", path[currVertex]);
                if (currVertex > 0) {
                    printf("->");
                }
            }
